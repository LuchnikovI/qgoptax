...
class RAdam(Optimizer):
    ...
    def _create_state(self,
        ...
        state = (jnp.zeros_like(param), jnp.zeros((*param.shape[:-2], 1, 1), dtype=param.dtype))
        if self.ams:
    ...
    def _apply(self,
        ...
        v = self.beta2 * state[1] + (1 - self.beta2) * self.manifold.inner(param, rgrad, rgrad)
        if self.ams:
            v_hat = jnp.maximum(jnp.real(v), jnp.real(state[2]))
        
        # Bias correction
        lr_corr = self.learning_rate * jnp.sqrt(1 - self.beta2 ** (iter + 1)) / (1 - self.beta1 ** (iter + 1))

        if self.ams:
            search_dir = -lr_corr * momentum / (jnp.sqrt(v_hat) + self.eps)
            param, momentum = self.manifold.retraction_transport(param, momentum, search_dir)
